...
**This block is generated from `section-01/beats.ts`.** It drifted once and
`npm run drift` is what catches it; the build is the authority for spoken lines.

### Act 1 — the screen, and what is behind it (beats 1–6) · **S-01**

The one object in this subject that needs no explaining, and then what is
behind it. The hook is a **disproportion**, not a number: one small word
against forty-five floors, 288 specialists and 336 trips. Nothing here is
explained -- beats 13-23 explain all of it. Show, then name.
...
>
> **6.** *(A counter runs up to 336 under the whole thing)* Three hundred and thirty-six of them got pulled in. For that one word.

### Act 2 — the claim, and who it implicates (beats 7–11) · **S-01**, **S-02**

The statistic lands *after* the disproportion, so it reads as an accusation
rather than a lesson. Then the two sheets make it checkable.
...
>
> **10.** *(The number everybody quotes)* Both of them have that number. Only one of them is actually cheap to own.
>
> **11.** *(Level above, unequal below)* Same five percent. Eight times the machine.

### Act 3 — the promise (beats 12–12) · **S-03**

Sanderson's SoME criterion: clear within thirty seconds why to stay. This
now lands at about 0:50, because the first twenty-eight seconds are spent
earning the question rather than posing it.

> **12.** *(The promise)* By the end of this you’ll know exactly what that number buys you — and what it doesn’t. We’re going to follow one word all the way through.

### Act 4 — now the words, because now they are earned (beats 13–17) · **S-04**

Concrete before abstract. "Parameter" arrives only after the block has been
watched behaving twice.
...
### Act 5 — the bet (beats 18–21) · **S-05**

One of two hard bets in the whole video. Eight of 288 is 2.8% and nobody
guesses that low; Richland, Kornell & Kao (2009) -- the wrong guess still
helps. The 288 do not move between the question and the answer.
...
### Act 6 — the name and the wall (beats 22–23) · **S-12**, **S-14**
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | a screen | — | a chat window; a question types itself in | `Chat`, model name small on the chrome | **`GLM-5.3-Flash`** | S-01 |
...
| 6 | behind the screen | — | a counter runs up under the whole thing | `Counter` → 336 | **`336 — for one word`** | S-01 |
| 7 | the sheet | **back out** | the machinery clears and two model sheets arrive side by side — **left coarse, right fine** — five percent live on each | two `Block`s, 5% lit, different grain | — | S-01 |
| 8 | the sheet | — | one chip slides in under the left block | `MachineBox` ×1 | — | S-01 |
| 9 | the sheet | — | seven more cards stack under the right block | `Rig` ×8 | — | S-01 |
| 10 | the sheet | — | the quoted phrase writes itself across both blocks | handwritten note | **`only 5% active`** | S-02 |
| 11 | the sheet | — | the blocks hold, visibly unequal underneath | both blocks, 1 vs 4 cards | — | S-02 |
| 12 | the sheet | — | the left block and its single chip slide out of frame | the eight-chip `Block` left | — | S-03 |
...
| 16 | inside | **push in** | the camera moves in on the lit region | the block filling frame | — | S-04 |
| 17 | inside | — | 288 separate pieces appear in rows | `Hospital` staffed | — | S-04 |
| 18 | inside | — | everything stops; the question holds on screen | the 288, still | **`how many run?`** | S-05 |
| 19 | inside | — | eight of them light, in place | `Hospital` lit ×8 | — | S-05 |
| 20 | inside | — | the other 280 stay flat and grey | `Hospital` quiet | — | S-05 |
| 21 | inside | — | the eight line up with beat 9's live sliver | brace between them | — | S-14 |
| 22 | inside | — | the arrangement holds and takes its name | plaque | **`Mixture of Experts`** | S-12 |
| 23 | inside, one part | **push in** | everything recedes but the eight | `Hospital` focus | — | S-14 |

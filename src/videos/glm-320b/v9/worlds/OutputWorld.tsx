...
export function OutputWorld({ beat }: { beat: number }) {
  ...
  return (
    <motion.section className="v9-world v9-output-world" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      {!payoff ? (
        <ChapterHeading eyebrow="08 · OUTPUT · NEXT TOKEN" note="We deliberately made “it” the final token in the prompt, so the same position we followed can now teach next-token prediction.">
          The final representation asks: <mark>what token comes next?</mark>
        </ChapterHeading>
      ) : null}
      ...
      {vocab ? (
        <div className="v9-output-canvas">
          ...
          <div className="v9-final-position">
            ...
            <div>{SENTENCE.split(' ').map((word, index, all) => <span key={`${word}-${index}`} data-last={index === all.length - 1 ? 'true' : undefined}>{word}</span>)}</div>
          ...
          <div className="v9-output-arrow"><span>score possible next tokens</span><i /></div>

          <motion.div className="v9-output-vocab" initial={{ opacity: 0, x: '10cqw' }} animate={{ opacity: 1, x: 0 }}>
            <header><small>VOCABULARY OUTPUT</small><strong>{MODEL.vocabSize.toLocaleString()} possibilities</strong><span>the final representation is turned into a score for every vocabulary entry</span></header>
            ...
            {shortlist ? (
              <motion.div className="v9-candidate-list" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <small>TOP CANDIDATES · ILLUSTRATIVE</small>
          ...
          {beat === 94 ? <PaperNote className="v9-output-note" tone="yellow">We followed <b>it</b> on purpose. Because it is the last prompt token, its final-position representation is the one used to score the next token.</PaperNote> : null}
          {beat === 95 ? <PaperNote className="v9-output-note">The model does not search a dictionary for a sentence. It scores the vocabulary for the <b>next token</b>.</PaperNote> : null}
      ...
      {loop ? (
        <motion.div className="v9-generation-loop" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div className="v9-loop-arrow"><span>longer sequence</span><i /><b>RUN THE STACK AGAIN</b></div>
          ...
          {montage ? <PaperNote className="v9-montage-note" tone="orange">This is autoregressive generation: after each chosen token, the sequence is longer and the model predicts again.</PaperNote> : null}
        </motion.div>
      ) : null}
      ...
      {payoff ? (
        <motion.div className="v9-final-paper" initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }}>
          ...
          <p>That is the core reason a 320B Mixture-of-Experts model can have a much smaller active parameter path for each token.</p>
        </motion.div>
      ) : null}
    </motion.section>
  )
